...
function createRkState(
  ...
  return {
    ...
    has_binomial:
      has_binomial ||
      null /* !=0 following parameters initialized for binomial */,
...
var rk_strerror = ["no error", "random device unavailable"];
...
function discnp_array(state, func, size, on, op, lock) {
  ...
  if (Sk.builtin.checkNone(size)) {
    ...
    var py_shape = new Sk.builtin.tuple(
      on.v.shape.map(function (x) {
    ...
    if (op1.length !== on1.length) {
      if (op1.length === 1) {
        ...
        throw new Sk.builtin.ValueError(
          "cannot broadcast n and p to a common shape"
        );
      }
    }
...
var $builtinmodule = function (name) {
  ...
  var randomState_c = function ($gbl, $loc) {
    ...
    $loc.get_state = new Sk.builtin.func(function (self) {
      ...
      var state = (obj = Sk.misceval.callsim(
        np.$d.asarray,
        new Sk.builtin.tuple(js_key),
        Sk.builtin.int_
      ));
    ...
    var js_random_integers = function (self, low, high, size) {
      ...
      return Sk.misceval.callsim(
        ...
        self,
        low,
        (sum = Sk.abstr.numberBinOp(high, new Sk.builtin.int_(1), "Add")),
        size
      );
    };
    js_random_integers.co_varnames = ["self", "low", "high", "size"];
    js_random_integers.$defaults = [
      Sk.builtin.none.none$,
      Sk.builtin.none.none$,
      Sk.builtin.none.none$,
    ];
    ...
    var js_binomial = function (self, n, p, size) {
      ...
      if (
        Sk.misceval.callsim(
          np.$d.any,
          Sk.misceval.callsim(np.$d.less, n, py_zero)
      ...
      if (
        Sk.misceval.callsim(
          np.$d.any,
          Sk.misceval.callsim(np.$d.less, p, py_zero)
        ) == Sk.builtin.bool.true$
      ) {
        throw new Sk.builtin.ValueError("p < 0");
      }
      if (
        Sk.misceval.callsim(
          np.$d.any,
          Sk.misceval.callsim(np.$d.greater, p, new Sk.builtin.int_(1))
        ) == Sk.builtin.bool.true$
      ) {
        throw new Sk.builtin.ValueError("p > 1");
      }
  ...
  mod[CLASS_RANDOMSTATE] = Sk.misceval.buildClass(
    mod,
    randomState_c,
    CLASS_RANDOMSTATE,
    []
